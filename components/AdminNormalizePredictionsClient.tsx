          ...
          <div className="flex items-center justify-between gap-4 border-b border-[var(--iberdrola-sky)] px-4 py-3">
            <div>
              <h2 className="text-base font-black text-[var(--iberdrola-forest)]">
                Predicciones — {search.trim() ? `${filteredRows.length} de ${rows.length}` : rows.length} participantes
              </h2>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <div className="relative">
                <span className="pointer-events-none absolute inset-y-0 left-2.5 flex items-center text-[var(--iberdrola-forest)]/40 text-xs">🔍</span>
                <input
                  type="text"
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  placeholder="Buscar nombre o valor…"
                  className="rounded-xl border border-[var(--iberdrola-sky)] pl-7 pr-3 py-2 text-xs font-semibold text-[var(--iberdrola-forest)] w-48 focus:outline-none focus:border-[var(--iberdrola-green)] focus:ring-1 focus:ring-[var(--iberdrola-green)]"
                />
                {search && (
                  <button onClick={() => setSearch("")}
                    className="absolute inset-y-0 right-2 flex items-center text-[var(--iberdrola-forest)]/40 hover:text-[var(--iberdrola-forest)]">
                    ✕
                  </button>
                )}
              </div>
              <button onClick={() => load(selectedQuestion, selectedPoolId, statusFilter)}
                className="rounded-xl border border-[var(--iberdrola-sky)] bg-white px-3 py-2 text-xs font-bold text-[var(--iberdrola-forest)] hover:bg-[var(--iberdrola-sand)]/50">
                🔄
              </button>
            </div>
          </div>
              ...
              <thead>
                <tr className="border-b border-[var(--iberdrola-sky)] bg-[var(--iberdrola-sand)]/30 text-xs font-black uppercase tracking-wide text-[var(--iberdrola-forest)]/55">
                  <th className="px-4 py-2.5 text-left w-8">
                    <input type="checkbox"
                      checked={filteredRows.length > 0 && filteredRows.every(r => selected.has(r.entry_id))}
                      onChange={toggleAll}
